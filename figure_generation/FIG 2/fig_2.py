...
# ===========================================================================
df_patients = df[['classifier_new_id', 'sex (M=0, F=1)', 'age_at_scans_years',
                  'epilepsy_onset_years', 'histo_diagnosis', 'hemi']].copy()
...
df_patients['histo_string'] = df_patients.histo_diagnosis.apply(lambda x: 'FCD III' if x == 1 else 'HS only')
...
VIOLIN_PALETTE = {'FCD III': '#E07B39', 'HS only': '#2C6BAC'}
ALPHA = 0.7
DOT_SIZE = 28
...
outer_gs = gridspec.GridSpec(2, 2, figure=fig, hspace=0.72, wspace=0.10,
...
for j, label in enumerate(labels_mat[:-1]):
    ax_bl.text(j + 0.5, n_mat - 0.85, label, ha='center', va='bottom',
               fontsize=9, color='gray', transform=ax_bl.transData)
...
# ═══════════════════════════════════════════════════════════════════════════
order = ['FCD III', 'HS only']
violin_colors = [VIOLIN_PALETTE[k] for k in order]
sns.violinplot(data=df1, x='histo_string', y='z_score', order=order, palette=violin_colors,
               inner=None, cut=0, linewidth=0.8, ax=ax_br, alpha=0.55, saturation=0.85)
...
g1 = df1.loc[df1['histo_string'] == 'FCD III', 'z_score'].dropna()
...
print(f"Panel D  FCD III vs HS only:   U = {U_stat:.0f}, {p_vio_str}")
